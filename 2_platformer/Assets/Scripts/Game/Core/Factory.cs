...
public class Factory 
{
                ...
                if (sectionsNode.Name == "baseObject") 
				{
					...
					foreach (XmlNode componentNode in sectionsNode) //components
					{
						if (componentNode.Name == "parameter") 
						{
							...
							if (name1 == "show")
							{
								temp.parameters.show = Convert.ToBoolean (value1, CultureInfo.InvariantCulture);
							}
							if (name1 == "x")
							{
								temp.parameters.x = float.Parse (value1, CultureInfo.InvariantCulture);
							}
							if (name1 == "y")
							{
								temp.parameters.y = float.Parse (value1, CultureInfo.InvariantCulture);
							}
							if (name1 == "width")
							{
								temp.parameters.width = float.Parse (value1, CultureInfo.InvariantCulture);
							}
							if (name1 == "height")
							{
								temp.parameters.height = float.Parse (value1, CultureInfo.InvariantCulture);
							}
							if (name1 == "move")
							{
								temp.parameters.move = Convert.ToBoolean (value1, CultureInfo.InvariantCulture);
							}
						}
						if (componentNode.Name == "component") 
						{
							...
							if (componentNode.Attributes ["name"].Value == "Phys") 
							{
								...
								foreach (XmlNode listNode in componentNode) //list
								{
									if (listNode.Name == "nx") 
									{
										nx = float.Parse(listNode.InnerText, CultureInfo.InvariantCulture);
									}
									if (listNode.Name == "ny") 
									{
										ny = float.Parse(listNode.InnerText, CultureInfo.InvariantCulture);
									}
								}
							...
							if (componentNode.Attributes ["name"].Value == "Collider") 
							{
								...
								foreach (XmlNode listNode in componentNode) //list
								{
									if (listNode.Name == "v") 
									{
										vx = float.Parse(listNode.Attributes["vx"].Value, CultureInfo.InvariantCulture);
									
										vy = float.Parse(listNode.Attributes["vy"].Value, CultureInfo.InvariantCulture);
									
										vz = float.Parse(listNode.Attributes["vz"].Value, CultureInfo.InvariantCulture);
									
										vw = float.Parse(listNode.Attributes["vw"].Value, CultureInfo.InvariantCulture);
							...
							if (componentNode.Attributes ["name"].Value == "Move")
							{
								...
								foreach (XmlNode listNode in componentNode) //list
								{
									if (listNode.Name == "vx") 
									{
										vx = float.Parse (listNode.InnerText, CultureInfo.InvariantCulture);
									}
									if (listNode.Name == "vy") 
									{
										vy = float.Parse (listNode.InnerText, CultureInfo.InvariantCulture);
									}
								}
	...
	{
        return MonoBehaviour.Instantiate(Resources.Load<GameObject>(obj));
