	...
	private bool action(ObjectContainer parameters)
	{
		...
		for (int i = 0; i < actions.Count; i++) 
		{
			...
			if  (actions [i].name == "cameraInSquare") 
			{
                ...
                float x = float.Parse (actions [i].listOfParameters [0], CultureInfo.InvariantCulture);
				float y = float.Parse (actions [i].listOfParameters [1], CultureInfo.InvariantCulture);
				float wid = float.Parse (actions [i].listOfParameters [2], CultureInfo.InvariantCulture) / 2;
				float hei = float.Parse (actions [i].listOfParameters [3], CultureInfo.InvariantCulture) / 2;
			...
			if (actions [i].name == "pause") 
			{
				//"pause:3"
				//Debug.Log(actions[i].listOfParameters[0]);
				float a = float.Parse(actions[i].listOfParameters[0], CultureInfo.InvariantCulture);
				a -= 0.1f;
